import os, sys, re, threading, asyncio, queue as Q, socket, time, base64
from datetime import datetime

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_TEMP_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'temp')
_UPLOAD_DIR = os.path.join(_TEMP_DIR, 'uploaded')
...
TEXT_FILE_EXTS = {
    ...
    '.toml', '.ini', '.cfg', '.conf', '.log', '.csv', '.tsv', '.xml', '.html', '.htm', '.css',
    '.sql', '.sh', '.bash', '.zsh', '.ps1', '.bat', '.java', '.c', '.cc', '.cpp', '.h', '.hpp',
    '.go', '.rs', '.php', '.rb', '.swift', '.kt', '.m', '.mm'
}
...
def _build_prompt_with_uploads(prompt, files):
    if not files:
        return prompt
    ...
    for f in files:
        ...
        saved = os.path.join(_UPLOAD_DIR, f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{safe}")
...
async def _collect_telegram_uploads(message):
    files = []
...
async def _stream(dq, msg):
    ...
    while True:
        ...
        if display != last_text:
            try:
                await msg.edit_text(_to_html(display), parse_mode='HTML')
            except Exception:
                try:
        ...
        if done:
            ...
            if show.strip():
                try:
                    await msg.edit_text(_to_html(show), parse_mode='HTML')
                except Exception:
                    try:
                        await msg.edit_text(show)
                    except Exception:
                        pass
            break
...
async def _enqueue_message(update, ctx):
    ...
    files = await _collect_telegram_uploads(update.message)
    prompt = _build_prompt_with_uploads(prompt, files)
...
async def handle_msg(update, ctx):
    await _enqueue_message(update, ctx)
...
if __name__ == '__main__':
    try:  # Single instance lock using socket
        ...
        _lock_sock.bind(('127.0.0.1', 19527))
    except OSError:
